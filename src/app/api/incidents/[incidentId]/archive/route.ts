...
export async function POST(_: Request, { params }: { params: { incidentId: string } }) {
  try {
    ...
    const { error } = await supabase.from("incidents").update({ is_active: false, updated_at: new Date().toISOString() }).eq("id", params.incidentId)
    if (error) {
    ...
    console.error("POST /api/incidents/[incidentId]/archive error", err)
